...
from csm.algorithm.clipper.clipper import * 
# import math
import re
    ...
    try:
        ...
        for l in lines:
            ...
            if len(vals) < 2:  
                ...
                poly.append(FloatPoint(float(vals[0]), float(vals[1])))
        if (len(poly)  > 2):
            polygons.append(poly)
        return polygons
    except:
...
def test1():
    ...
    SaveToFile('./solution1.txt', solution, scale)
...
#     subj, clip = [], [] 
    #load saved subject and clip polygons ...    
    subj = LoadFile('./subj3.txt')
    clip = LoadFile('./clip3.txt')
...
#     clip.append(RandomPoly(640 * scale, 480 * scale, 100))
    ...
    result = SimplifyPolygons(solution, pft)
#     result = []
#     for i in xrange(len(solution)):
#         result+=SimplifyPolygon(solution[i], pft)
    SaveToFile('./solution3.txt', result, scale)
