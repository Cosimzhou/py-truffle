...
#public中定义的成员可以直接使用
#=============================
# class oba(object): #公历基础构件
#  #private私有成员定义
#     wFtv  = ( #某月的第几个星期几,如第2个星期一指从月首开始顺序找到第2个“星期一”
...
# 
#   self.sFtv=s.split('|');
#   for(i=0;i<self.sFtv.length;i++) self.sFtv[i]=self.sFtv[i].split(',');
# 
#  },
...
# 
#   #按公历日期查找
#   for(i=0;i<self.sFtv[u.m-1].length;i++){ #公历节日或纪念日,遍历本月节日表
#    s=self.sFtv[u.m-1][i];
...
#   w2= m0 + w2 + u.week;
# 
#   for(i=0;i<self.wFtv.length;i++){
#    s=self.wFtv[i];
#    s2=s.substr(0,4);
#    if(s2!=w  and  s2!=w2) continue;
#    type=s.substr(4,1);
...
#public中定义的成员可以直接使用
#=============================
class obb(object): #农历基础构件
#private私有成员定义
    JNB  = None, #纪年表,由init初始化
    ...
    def __init__(self): #初始化
        ...
        s = ('-2069,45,0,夏,禹,,禹,-2024,10,0,夏,启,,启,-2014,25,0,夏,太康,,太康,-1986,14,0,夏,仲康,,仲康,-1972,28,0,夏,相,,相,-1944,2,0,夏,后羿,,后羿,-1942,38,0,夏,寒浞,,寒浞,-1904,21,0,夏,少康,,少康,-1883,17,0,夏,杼,,杼,-1866,26,0,夏,槐,,槐,-1840,18,0,夏,芒,,芒,-1822,16,0,夏,泄,,泄,-1806,59,0,夏,不降,,不降,-1747,21,0,夏,扃,,扃,-1726,21,0,夏,廑,,廑,'
  ...
  +'1862,13,0,清,穆宗,爱新觉罗载淳,同治,1875,34,0,清,德宗,爱新觉罗载湉,光绪,1909,3,0,清,无朝,爱新觉罗溥仪,宣统,1912,37,0,近、现代,中华民国,,民国,1949,9999,1948,当代,中国,,公历纪元')
        ...
        obb.JNB = s.split(',')
#       for(i=0;i<self.JNB.length;i+=7) { self.JNB[i]-=0; self.JNB[i+1]-=0; self.JNB[i+2]-=0; }



#     getNH:function(y){ #取年号
#      var i,j,c,s='', ob=self.JNB;
...
#   }
#   if(u.Lmc2=='正'){ #最后一月
#    if(d=='十二三十'  and  u.Ldn==30)  r.A += '除夕 ',r.Fjia = 1;
#    if(d=='十二廿九'  and  u.Ldn==29)  r.A += '除夕 ',r.Fjia = 1;
#    if(d=='十二廿三') r.B += '小年 ';
#   }
#   if(u.Ljq){
...
#   #农历杂节
#   var w,w2;
#   if(u.cur_dz>=0  and  u.cur_dz<81){ #数九
#    w = obb.numCn[floor(u.cur_dz/9)+1];
...
#   if(u.cur_xz>=20  and  u.cur_xz<30  and  w=='庚') r.B += '初伏 ';
#   if(u.cur_xz>=30  and  u.cur_xz<40  and  w=='庚') r.B += '中伏 ';
#   if(u.cur_lq>=0   and  u.cur_lq<10  and  w=='庚') r.B += '末伏 ';
#   if(u.cur_mz>=0   and  u.cur_mz<10  and  w=='丙') r.B += '入梅 ';
#   if(u.cur_xs>=0   and  u.cur_xs<12  and w2=='未') r.B += '出梅 ';
#  },
# 
...
#   jd += pty_zty2(jd2/36525)+J/pi/2; #本地真太阳时(使用低精度算法计算时差)
#   ob.bz_zty = JD.timeStr(jd);
# 
#   jd += 13/24; #转为前一日23点起算(原jd为本日中午12点起算)
#   var D = floor(jd), SC = int2( (jd-D)*12 ); #日数与时辰
# 
#   v = int2(k/12+6000000);  ob.bz_jn = self.Gan[v%10]+self.Zhi[v%12];
#   v = k+2+60000000;        ob.bz_jy = self.Gan[v%10]+self.Zhi[v%12];
#   v = D - 6 + 9000000;     ob.bz_jr = self.Gan[v%10]+self.Zhi[v%12];
#   v = (D-1)*12+90000000+SC;ob.bz_js = self.Gan[v%10]+self.Zhi[v%12];
# 
#   v-= SC, ob.bz_JS = ''; #全天纪时表
...
#   var d = pi/12;
#   var w=floor((jd+293)/365.2422*24) * d;
#   var a= self.qi_accurate ( w );
#   if(a-jd>5 ) return self.qi_accurate ( w-d );
#   if(a-jd<-5) return self.qi_accurate ( w+d );
#   return a;
#  },
#  so_accurate2: function(jd) { return self.so_accurate ( floor((jd+8)/29.5306) * pi*2 ); } #精朔
# 
# 
# 
# obb.init();
...
function Lunar(){
 var i;
 self.lun=new Array();
 for(i=0;i<31;i++) self.lun[i] = new Object();
 self.lun.dn=0;

 self.substr2=function(s,n,end){ #截串(网页设计对过长的文字做截处理)
  s=s.replace('(^\s*)|(\s*$)', "");
 ...
 self.yueLiCalc=function(By,Bm){
  ...
  self.w0= (Bd0 + J2000 +1+7000000)%7; #本月第一天的星期
  self.y = By; #公历年份
  self.m = Bm; #公历月分
  self.d0 =Bd0;
  self.dn= Bdn;
  ...
  #所属公历年对应的农历干支纪年
  c = By -1984 + 12000;
  self.Ly  = obb.Gan[c%10]+obb.Zhi[c%12];  #干支纪年
  self.ShX = obb.ShX[c%12]; #该年对应的生肖
  self.nianhao = obb.getNH(By);
  ...
  for(i=0,j=0;i<Bdn;i++){
    ...
    ob.m  = Bm;    #公历月
    ob.dn = Bdn;   #公历月天数
    ob.week0 = self.w0; #月首的星期
    ob.week  = (self.w0+i)%7; #当前日的星期
    ob.weeki = int2((self.w0+i)/7); #本日所在的周序号
    ob.weekN = int2((self.w0+Bdn-1)/7) + 1;  #本月的总周数
    JD.setFromJD(ob.d0+J2000); ob.d = JD.D; #公历日名称

    #农历月历
    if(!Shuoqi.ZQ.length || ob.d0<Shuoqi.ZQ[0] || ob.d0>=Shuoqi.ZQ[24]) #如果d0已在计算农历范围内则不再计算
      Shuoqi.calcY(ob.d0);
    var mk = int2( (ob.d0-Shuoqi.HS[0])/30 );  if(mk<13  and  Shuoqi.HS[mk+1]<=ob.d0) mk++; #农历所在月的序数

    ob.Ldi = ob.d0 - Shuoqi.HS[mk];   #距农历月首的编移量,0对应初一
    ob.Ldc = obb.rmc[ob.Ldi];      #农历日名称
    ob.cur_dz = ob.d0-Shuoqi.ZQ[0];   #距冬至的天数
    ob.cur_xz = ob.d0-Shuoqi.ZQ[12];  #距夏至的天数
    ob.cur_lq = ob.d0-Shuoqi.ZQ[15];  #距立秋的天数
    ob.cur_mz = ob.d0-Shuoqi.ZQ[11];  #距芒种的天数
    ob.cur_xs = ob.d0-Shuoqi.ZQ[13];  #距小暑的天数
    if(ob.d0==Shuoqi.HS[mk]||ob.d0==Bd0){ #月的信息
      ob.Lmc  = Shuoqi.ym[mk]; #月名称
      ob.Ldn  = Shuoqi.dx[mk]; #月大小
      ob.Lleap= (Shuoqi.leap and Shuoqi.leap==mk)?'闰':''; #闰状况
      ob.Lmc2 = mk<13?Shuoqi.ym[mk+1]:"未知"; #下个月名称,判断除夕时要用到
    }else{
      ob2=self.lun[i-1];
      ob.Lmc  = ob2.Lmc,   ob.Ldn  = ob2.Ldn;
      ob.Lleap= ob2.Lleap, ob.Lmc2 = ob2.Lmc2;
    }
    var qk=int2( (ob.d0-Shuoqi.ZQ[0]-7)/15.2184 ); if(qk<23  and  ob.d0>=Shuoqi.ZQ[qk+1]) qk++; #节气的取值范围是0-23
    if(ob.d0==Shuoqi.ZQ[qk]) ob.Ljq=obb.jqmc[qk];
    else ob.Ljq='';
    ...
    #干支纪年处理
    #以立春为界定年首
    D = Shuoqi.ZQ[3] + (ob.d0<Shuoqi.ZQ[3]?-365:0) + 365.25*16-35; #以立春为界定纪年
    ob.Lyear =  floor(D/365.2422+0.5); #农历纪年(10进制,1984年起算)
    #以下几行以正月初一定年首
    D = Shuoqi.HS[2]; #一般第3个月为春节
    for(j=0;j<14;j++){ #找春节
      if(Shuoqi.ym[j]!='正'||Shuoqi.leap==j and j) continue;
      D = Shuoqi.HS[j];
      if(ob.d0<D) { D-=365; break; } #无需再找下一个正月
    }
    D = D + 5810;  #计算该年春节与1984年平均春节(立春附近)相差天数估计
    ob.Lyear0 =  floor(D/365.2422+0.5); #农历纪年(10进制,1984年起算)
    ...
    mk = int2( (ob.d0 - Shuoqi.ZQ[0])/30.43685 );  if(mk<12  and  ob.d0>=Shuoqi.ZQ[2*mk+1]) mk++;  #相对大雪的月数计算,mk的取值范围0-12

    D = mk + int2( (Shuoqi.ZQ[12]+390)/365.2422 )*12 + 900000; #相对于1998年12月7(大雪)的月数,900000为正数基数
    ob.Lmonth = D%12;
    ...
    #星座
    mk = int2( (ob.d0-Shuoqi.ZQ[0]-15)/30.43685 );  if( mk<11  and  ob.d0>=Shuoqi.ZQ[2*mk+2] ) mk++; #星座所在月的序数,(如果j=13,ob.d0不会超过第14号中气)
 ...
 self.yueLiHTML=function(By,Bm,curJD){
  ...
  self.yueLiCalc(By,Bm);    #农历计算
  #年份处理
  c = self.nianhao +' 农历'+self.Ly+'年【'+self.ShX+'年】'; #干支纪年
  ...
  for(i=0;i<self.dn;i++){ #遍历本月各日(公历)
    #生成i日的日历页面
    ob = self.lun[i];
    if(!i){ for(j=0;j<self.w0;j++) cr+='<td'+sty_body+'></td>'; } #首行前面的空单元格

    c = '', isM = ''; #文字格式控制项
    if(ob.A)   c += '<font color=red>' +self.substr2(ob.A,4,'..')+'</font>';
    if(!c  and  ob.B)   c = '<font color=blue>'+self.substr2(ob.B,4,'..')+'</font>';
    if(!c  and  ob.Ldc=="初一") c = ob.Lleap + ob.Lmc +'月'+ (ob.Ldn==30?'大':'小'); #农历历月(闰月及大小等)
    ...
    if(i==self.dn-1) { for(j=0;j<6-ob.week;j++) cr+='<td'+sty_body+'></td>'; } #末行后面的空单元格
    if(i==self.dn-1||ob.week==6) ta0+='<tr>'+cr+'</tr>', cr="";
  }
  self.pg1='<table border=0 cellpadding=3 cellspacing=1 width="100%">'+ta0+'</table>';
...
#  Shuoqi.calcY( int2((y-2000)*365.2422+180) );
#  for(i=0;i<14;i++){
#   if(Shuoqi.HS[i+1]>Shuoqi.ZQ[24]) break; #已包含下一年的冬至
#   if(Shuoqi.leap  and  i==Shuoqi.leap) s1='闰'; else s1='·';
#   s1 += Shuoqi.ym[i]; if(s1.length<3) s1 += '月';
#   s1 += Shuoqi.dx[i]>29?'大':'小';
#   s1 += ' '+JD.JD2str(Shuoqi.HS[i]+J2000).substr(6,5);
# 
#   v = obb.so_accurate2(Shuoqi.HS[i]);
#   s2 = '('+ JD.JD2str(v+J2000).substr(9,11)+')';
#   if(int2(v+0.5)!=Shuoqi.HS[i]) s2 = '<font color=red>'+s2+'</font>';
#   #v=(v+0.5+J2000)%1; if(v>0.5) v=1-v; if(v<8/1440) s2 = '<u>'+s2+'</u>'; #对靠近0点的加注
#   s1 += s2;
# 
#   for(j=-2;j<24;j++){
#     if(j>=0)  qi=Shuoqi.ZQ[j];
#     if(j==-1) qi=Shuoqi.ZQ.pe1;
#     if(j==-2) qi=Shuoqi.ZQ.pe2;
# 
#     if(qi<Shuoqi.HS[i] || qi>=Shuoqi.HS[i+1]) continue;
...
if __name__ == '__main__':
    jd = 5485#-655957#5156 #2457074#2456685#5522#2456693#(
    ssq= Shuoqi()
    ssq.calcY(jd)
    print jd
