...
def evSSB(t): #地球的SSB速度, t儒略世纪, 速度单位AU/世纪, 平均误差4*10^-8AU/日
    J = (
...
def epSSB(t): #地心SSB坐标,t儒略世纪数,4位有效数字
    t /= 10
    x=0 
    y=0 
    z=0 
    p=epTab
    for i in xrange(0, epTab.length, 6):
    ...
    E = -84381.448/rad
    return ( x, z*sin(E)+y*cos(E), z*cos(E)-y*sin(E) )

"""=================================引力偏转=========================================
=================================================================================="""
def ylpz(z,a): #引力偏转,z天体赤道坐标,a太阳赤道坐标
    r = [z[0],z[1],z[2]]
    d = z[0]-a[0]
...
xz88= (#88星座表。结构：汉语+缩写,面积(平方度),赤经 (时分),赤纬(度分),象限角 族 星座英文名
...
#  var i, s, n0,n1,n2,n3, 
    r=''
    for i in xrange(HXK.length): #遍历所有子库
        s = HXK[i];
    ...
    for i in xrange(0, xz88.length, 5): #提取星座中心位置
...
def getHXK(s,all): #提取并格式化恒星库(把度分秒、角分秒转为弧度),all=1表示全部取出
#  var i,k,  r=new Array();
    s = s.replace('/\r\n/g', '#')       #把换行符号看作#号
    sn = s.indexOf('#')+1
    ...
    for i in xrange(0, s.length, 8):
        if(not s[i] or s[i].length<5): continue
        if(s[i].substr(0,1)!='*' and not all): continue
...
def hxCalc(t, F, Q, lx, L, fa): #多颗同时计算,t儒略世纪TD,只算章动周期在于Q天(为0不限制)
    s = ''
    z = []
